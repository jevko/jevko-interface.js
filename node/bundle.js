...
var extendedJevkoml = (jevko, opts) => {
  ...
  const markdown = async (jevko2) => {
    const { tag, suffix, ...rest } = jevko2;
    const proc = run({
    ...
    if (cssIncluded === false) {
      cssIncluded = true;
      return `<style>${import_static.default["pandoc-highlighting.css"]}</style>` + await readFullStdoutText(proc);
    }
